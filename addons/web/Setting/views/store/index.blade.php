...
<script>
var routes = {
    index: 'setting/store/index',
    create: 'setting/store/create',
    delete: 'setting/store/delete',
    edit: 'setting/store/edit',
    show: 'setting/store/show',
    export: 'setting/store/export',
};
var $table = null;
...
function actionLink(action, id) {
    ...
    if(action == 'create') {
        formBox('新建', app.url(routes.create), 'user-group-form', function(res) {
            if(res.status) {
    ...
    if(action == 'edit') {
        ...
        formBox('编辑', app.url(routes.edit, {id: id}), 'user-group-form', function(res) {
            if(res.status) {
                $.toastr('success', res.data, '提醒');
                $table.jqGrid('setGridParam', {
                    postData: params,

<div class="form-group">
    ...
    <select class="form-control input-sm" data-toggle="redirect" id='role_id' name='role_id' rel="{{url($atcion, $query)}}">
        @foreach($roles as $key => $role)
            <option value="{{$role->id}}" @if($query['role_id'] == $role->id) selected @endif>{{$role->layer_space}}{{$role->title}}</option>
        @endforeach
    </select>
</div>
&nbsp;
<div class="form-group">
    ...
    <select class="form-control input-sm" data-toggle="redirect" id='clone_id' name='clone_id' rel="{{url($atcion, $query)}}">
        ...
        @foreach($roles as $key => $role)
            <option value="{{$role->id}}" @if($query['clone_id'] == $role->id) selected @endif>{{$role->layer_space}}{{$role->title}}</option>
        @endforeach
    </select>
</div>
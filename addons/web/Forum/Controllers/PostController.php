<?php namespace Aike\Web\Forum\Controllers;

use DB;
use Auth;
use Input;

use Aike\Web\Forum\Forum;
use Aike\Web\Forum\ForumPost;
...
class PostController extends DefaultController
{
    ...
    public function indexAction()
    {
        ...
        if ($level < 4) {
            ...
            //$forum->whereRaw('(FIND_IN_SET(?,forum.scope_user_id) OR FIND_IN_SET(?,forum.scope_role_id) OR FIND_IN_SET(?,forum.scope_department_id))', [Auth::id(), Auth::user()->role_id, Auth::user()->department_id]);
        }

        $forum
        //->LeftJoin('forum_post', 'forum_post.forum_id', '=', 'forum.id')
        //->whereRaw('forum_post.parent_id = 0')
        ->selectRaw('forum.*') //count(forum_post.id) as count
        ->groupBy('forum.id');

...
class EventController extends DefaultController
{
    ...
    {
        if ($gets = Input::get()) {
            $events = Calendar::getRangeEvents($gets['calendar_id'], $gets['start'], $gets['end']);
    ...
    public function itemsAction()
    {
        ...
        ->where(function ($q) use ($start, $end) {
            $q->whereBetween('calendar_object.firstoccurence', [$start, $end]);
            $q->orWhereBetween('calendar_object.lastoccurence', [$start, $end]);
        })
    ...
    public function shareAction()
    {
        ...
        foreach ($events as &$event) {
            $event['title'] = '['.$share[$event['id']].']'.$event['title'];
        }
        return response()->json($events);
    }

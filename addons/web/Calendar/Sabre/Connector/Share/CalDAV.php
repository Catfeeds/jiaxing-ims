<?php namespace Aike\Web\Calendar\Sabre\Connector\Share;

use Aike\Web\Calendar\Sabre\Connector;

use Auth;

use Aike\Web\Index\Share;
...
class CalDAV extends Connector\CalDAV implements \Sabre\CalDAV\Backend\SharingSupport
{
    ...
    public function getCalendarsForUser($principalUri)
    {
        ...
        $shareRows = Share::getItemsSourceBy(['calendar'], $principal['id']);

        foreach ($shareRows as $shareRow) {
            // get the original calendar
            $calendarShareRow = Calendar::getCalendar($shareRow['source_id'], false);
            ...
            $shareRow['displayname'] = '['.$shareRow['nickname'].']'.$calendarShareRow['displayname'];
        ...
        // 获取分享记录
        $shareSource = Share::getItemsSourceBy(['event'], Auth::id());
        $sharedEvent = array(
            'id'  => 'share-events',
            'uri' => 'share-events',
            'principaluri' => $principalUri,
    ...
    {
        return new \Aike\Web\Calendar\Sabre\Connector\Principal();
    }
}

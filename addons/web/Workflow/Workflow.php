...
class Workflow extends BaseModel
{
    ...
    public function getfilterUsers($type, $department_id, $role_id, $user_id)
    {
        ...
        $rows = $model->from('user', 'id,role_id,department_id,username,nickname,mobile')
    ...
    public function saveForm($post, $draft = false)
    {
        ...
        foreach ($work_form_data as $key => $value) {
            ...
            if ($value['class'] == 'listview') {
                ...
                if (count($post[$key])) {
                    // 重新组合列表顺序
                    foreach ($post[$key] as $_post) {
                        $_data[] = $_post;
                    }
                    $data[$key] = json_encode($_data);
                }
    ...
    public function notification($post)
    {
        ...
        // 取得要通知的人信息
        $_users = DB::table('user')->whereIn('id', $notification_user)->get(['id', 'username', 'email', 'mobile']);
        ...
        foreach ($notification_type as $i => $type) {
            ...
            if (is_array($type)) {
                ...
                // 获取短信通知人
                if (in_array(1, $type) && $_users[$user_id]['mobile']) {
                    $users[$i]['mobile'] = $_users[$user_id]['mobile'];
                }
        ...
        foreach ($users as $user) {
            ...
            // 即时通知
            $notify->bigant([$user['username']], $subject, $body);

            // 短信通知
            $notify->sms([$user['mobile']], $subject);
            
            // 邮件通知
            // notify()->mail([$user['email']], $subject, $body);
        }
        return true;
    }
        ...
        foreach ($work_form_data as $key => $item) {
            ...
            switch ($class) {
                ...
                case "text":
                    ...
                    $item_value = $print == true ? self::formDisplay($item_value, $attribute) : \Form::text($item_name, $item_value, $attribute);
                    break;
                ...
                case "textarea":
                    $attribute['class'] = self::formRequired($readonly, $required);
                    $item_value = $print == true ? self::formDisplay(nl2br($item_value), $attribute) : \Form::textarea($item_name, $item_value, $attribute);
                    break;
                ...
                case "checkbox":
                    ...
                    $item_value = $print == true ? $item_value : '<input '.self::formAttribute($attribute).$readonly.$checked.' />';
                    break;
                    ...
                    if ($tag == 'input') {
                        switch ($dataField) {
                           ...
                           // 当前用户辅助职位
                            ...
                            // 当前用户姓名+日期
                            case "sys_nickname_date":
                                $auto_value = Auth::user()->nickname.date(' Y-m-d');
                                break;

                            // 当前用户姓名+日期+时间
                            case "sys_nickname_datetime":
                                $auto_value = Auth::user()->nickname.date(' Y-m-d H:i:s');
                                break;

                            // 当前业务员姓名
                            case "sys_salesman_name":
                                $user = DB::table('user')->where('id', Auth::user()->salesman_id)->first(['id', 'nickname']);
                                $auto_value = $user['nickname'];
                                break;
                ...
                case "listview":
                    ...
                    $thead[] = '<thead><tr><th style="width:40px;white-space:nowrap;" align="center">序号</th>';
                    $tfoot[] = '<tfoot><tr><th style="white-space:nowrap;" align="center">合计</th>';
                    ...
                    $field = array('readonly'=>(int)$readonly,'size'=>$field_size,'sum'=>$field_sum,'type'=>$field_type,'value'=>$field_value);
    ...
    public static function updateTable($workId)
    {
        $structure = array(
            ...
            'select'   => "varchar(120) NOT NULL",
            'text'     => "varchar(255) NOT NULL",
            'textarea' => "text NOT NULL",
            'auto'     => "varchar(100) NOT NULL",
            'listview' => "longtext NOT NULL",

...
class WorkflowController extends DefaultController
{
    ...
    public function indexAction($flag = 'index')
    {
        ...
        $search = search_form([
            ...
            ['text','user.nickname','发起人'],
            ['text','handle_user','主办人'],
        ...
        // 数据总计
        $total = $model->distinct()->count('wp.id');
        $rows = $model->groupBy('wp.id')
        ->forPage($page, 15)
        ->select(['wp.*', 'wpd.process_id'])
        ->get();
        ...
        $categorys = WorkflowCategory::where('status', 1)->get(['id','title']);
        ...
        if (Request::method() == 'POST') {
            ...
            */
            ...
            foreach ($rows as $key => &$row) {
                ...
                $row['start_user_id'] = get_user($row['start_user_id'], 'nickname', 0);
    ...
    {
        $works = Workflow::whereRaw('FIND_IN_SET(?, auth_user) OR FIND_IN_SET(?, auth_role) OR FIND_IN_SET(?, auth_department)', [Auth::id(), Auth::user()->role_id, Auth::user()->department_id])
        ->get();

        $rows = $categorys_count = $categorys = [];
        foreach ($works as $rowId => $row) {
            $rows[$row->category_id][$rowId] = $row;
            $categorys_count[$row->category_id]++;
        }

        $j = 0;

        $_categorys = WorkflowCategory::orderBy('sort', 'ASC')->get();

        foreach ($_categorys as $i => $category) {
            if ($categorys_count[$category->id]) {
                if ($i % 3 == false) {
                    $j++;
                }
                $categorys[$j][] = $category;
    ...
    {
        $process_id = (int)Input::get('process_id');
    ...
    public function lastAction()
    {
        ...
        $work_id = (int)Input::get('work_id');
        $step_id = (int)Input::get('step_id');

...
class LossController extends DefaultController
{
    // 首页
    public function homeAction()
    {
        ...
        $model = Stock::where('type_id', 3)
        ->where('status', 1)
        ->whereRaw("DATE_FORMAT(date,'%Y') = DATE_FORMAT(CURDATE(),'%Y')");
        if ($query['store_id']) {
            $model->where('stock.store_id', $query['store_id']);
        }
    ...
    public function createAction()
    {
        ...
        if (Request::method() == 'POST') {
            ...
            return $this->json('恭喜你，报损出库更新成功。', true);
        }
    ...
    {
        $id   = Input::get('id');
        $node = Input::get('node', 'stock.loss');
    ...
    public function invalidEditAction()
    {
        ...
        if (Request::method() == 'POST') {
            if ($row->id) {
                ...
                return $this->json('恭喜你，报损出库作废成功。', true);
            } else {
                return $this->json('很抱歉，报损出库不存在。');
            }
        }
    ...
    public function deleteAction()
    {
        if (Request::method() == 'POST') {
            ...
            return $this->json('恭喜你，报损出库删除成功。', true);
        }
    }
}

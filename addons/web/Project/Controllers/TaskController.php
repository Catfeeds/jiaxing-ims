...
class TaskController extends DefaultController
{
    ...
    public function indexAction(Request $request)
    {
        ...
        if ($request->ajax() && $request->wantsJson()) {
            ...
            $tasks = array_tree($tasks, 'text');
            $tasks = reduce_tree($tasks);
            $json['data'] = $tasks;
        ...
        // 返回页面
        $referer = session()->get('referer_'.$request->module().'_'.$request->controller().'_index');
    ...
    public function data($search)
    {
        ...
        $model = Task::with(['users' => function ($q) {
            $q->select(['user.id','user.nickname as user_name']);
        }]);
        ...
        $_tasks = $model->select(['project_task.*','user.nickname as user_name', 'project.user_id as project_user_id'])
        ->orderBy('project_task.sort', 'asc')
        ->orderBy('project_task.id', 'asc')
        ->get()->toArray();
        ...
        foreach ($_tasks as $_task) {
            ...
            $_task['text']       = $_task['name'];
            $_task['parent']     = $_task['parent_id'];
            $_task['users']      = join(',', array_pluck($_task['users'], 'user_name'));
            $_task['open']       = true;
            $_task['loaded']     = true;
            $_task['expanded']   = true;
    ...
    public function addAction(Request $request)
    {
        if ($request->method() == 'POST') {
            ...
            if ($gets['is_item'] == '0') {
                ...
                $task->user_name  = get_user($task->user_id, 'nickname', false);
    ...
    public function editAction(Request $request)
    {
        if ($request->method() == 'POST') {
            ...
            $gets['progress'] = isset($gets['progress']) ? 1 : $gets['progress'];
        ...
        $tasks = Task::where('parent_id', $task->id)
        ->orderBy('id', 'desc')
        ->get();

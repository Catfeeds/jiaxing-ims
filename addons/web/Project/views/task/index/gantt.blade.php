...
gantt.config.columns = [
	{name:"text", label:"任务列表", tree:true, width:'*', resize: true}
];
...
gantt.templates.tooltip_text = function(start, end, task) {
	if(task.type == 'task' || task.type == 'subtask') {
		return '<div>任务: '+task.text+'</div><div>执行者: ' + (task.user_name || '无') + '</div><div>参与者: ' + (task.users || '无') + '</div><div>开始时间: ' + gantt.templates.tooltip_date_format(start) + '</div><div>结束时间: '+gantt.templates.tooltip_date_format(end) + '</div><div>备注: '+task.remark+'</div>';
	}
};
...
gantt._do_autosize = function() {
	...
	$('#gantt-view').height(iframeHeight - height - 77 + 'px');

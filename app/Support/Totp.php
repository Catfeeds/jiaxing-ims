...
class Totp
{
    ...
    {
        //http://shenghua.app/app/api/qrcode?size=6&data=

        $encoder = url('index/api/qrcode', ['size'=>6,'data'=>'']);
        
        //$encoder = "http://api.k780.com:88/?app=qr.get&level=M&size=6&data=";

        $encoderURL = sprintf("%sotpauth://totp/%s@%s%%3Fsecret%%3D%s", $encoder, $user, $hostname, $secret);
        return $encoderURL;
        /*
        $encoder = url('app/api/qrcode').'?level=3&size=120&data=';
        $encoderURL = sprintf("%sotpauth://totp/%s@%s%%3Fsecret%%3D%s", $encoder, $user, $hostname, $secret);
        return $encoderURL;
        */
    }
